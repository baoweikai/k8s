...
do
    if [ $APISERVER_IP != master ]; then
		sshpass -p $password scp -o "StrictHostKeyChecking no" -r '/mnt/k8s/shell/init.sh' $username'@'$master':/tmp/init.sh'
		sshpass -p $password ssh -o "StrictHostKeyChecking no" $username'@'$master sudo /tmp/init.sh ${VERSION}
...
do
    ...
    sshpass -p ${password} ssh -o "StrictHostKeyChecking no" ${username}'@'${worker} sudo /tmp/init.sh ${VERSION}
